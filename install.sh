...
echo "Keploy path: ${KEPLOY_PATH}"
# Generate app binary
echo "Directory contents:"
ls -la

# Debug: Check if test sets exist
echo "Checking for test-sets in ${KEPLOY_PATH}"
find ${KEPLOY_PATH} -type d -name "test-sets" -o -name "test-set-*"
...
if [[ "$COMMAND" =~ .*"go".* ]]; then
  ...
  echo "Running: sudo -E keploy test -c \"./application\" --delay ${DELAY} --path ${KEPLOY_PATH}"
  ...
  npm install
  echo 'Test Mode Starting 🎉'
  echo "Running: sudo -E keploy test -c \"${COMMAND}\" --delay ${DELAY} --path ${KEPLOY_PATH}"
  ...
  echo 'Test Mode Starting 🎉'
  echo "Running: sudo -E keploy test -c \"${COMMAND}\" --delay ${DELAY} --path ${KEPLOY_PATH} --containerName ${CONTAINER_NAME} --buildDelay ${BUILD_DELAY}"
  ...
  echo 'Test Mode Starting 🎉'
  echo "Running: sudo -E keploy test -c \"${COMMAND}\" --delay ${DELAY} --path ${KEPLOY_PATH} --buildDelay ${BUILD_DELAY}"
